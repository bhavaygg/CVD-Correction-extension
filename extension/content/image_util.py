...
def CVD_Stim(img, CVD_type, simple_linear_transform=False):
    ...
    if len(sizeImg) == 3:
        ...
        imgR = img[:, 2]
    GAMMA = 2.2
    imgRGBVec = np.concatenate(
        ([imgR.flatten()], [imgG.flatten()], [imgB.flatten()]), axis=0
    )
...
@app.route('/gen_cvd/', methods=['GET', 'POST'])
def generate_cvd():
    ...
    # image = np.array(Image.open(requests.get(sys.arg[1], stream=True).raw))
    ...
    # proto_corrected = cv2.cvtColor(
    #     np.uint8(CVD_Stim(image, "Protanopia", simple_linear_transform=True)),
    #     cv2.COLOR_BGR2LAB,
    # )
    ...
    # cv2.imwrite(
    #     "baseline_corrected_protonopia.jpg",
    #     np.uint8(cv2.cvtColor(np.uint8(proto_corrected), cv2.COLOR_LAB2BGR)),
    # )
